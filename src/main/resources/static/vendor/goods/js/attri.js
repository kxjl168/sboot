...
*/
	...
	$('#myModal_attr').on('show.bs.modal', function(e) {
		 ...
		 */
		// $(".help-block").html('');
		// $("#mform_attri").data('bootstrapValidator').resetForm(true);
	});
	

});

function attr_actiondone()
{
	doquery();
	}
...
function PersonnelInformationTable() {
	...
	$('#table_list').bootstrapTable({
		url : '/manager/attri/list', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
	...
	"click #update" : function(e, value, row, index) {
		$.ajax({
			...
			success : function(response) {
				...
				$("#isSpecification").find("option[value='" + response.isSpecification + "']").attr(
						"selected", true);

				$("#mandatory").find("option[value='" + response.mandatory + "']").attr(
						"selected", true);
